  ...
  return (
    <div className="space-y-5 sm:space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl sm:text-2xl font-bold text-gray-900">Gestão de Itens de Enxoval</h2>
          <p className="text-sm text-gray-600">Gerencie os itens disponíveis para pedidos</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        {linenItems.map((item) => (
          <div key={item.id} className="bg-white rounded-xl p-4 sm:p-6 shadow-sm border border-gray-100 hover:shadow-md transition-all">
            <div className="flex items-start justify-between mb-3 sm:mb-4">
              <div className="flex items-center space-x-3">
                <div className={`p-2 rounded-lg ${item.currentStock <= item.minimumStock ? 'bg-red-100' : 'bg-blue-100'}`}>
                  <Package className={`w-5 h-5 sm:w-6 sm:h-6 ${item.currentStock <= item.minimumStock ? 'text-red-600' : 'text-blue-600'}`} />
                </div>
                <div>
